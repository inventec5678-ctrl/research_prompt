# BTC 15分K 交易策略研究框架

## 目標

找出能通過以下門檻的 alpha 策略：
- WR ≥ 60%
- PF ≥ 1.8
- DD ≤ 25%
- Sharpe ≥ 1.5
- 月交易 ≥ 100

---

## 版本變更日誌 (Changelog)

### v2.0 — 2026-04-18 14:44
**強制雙向獲利 + Exit 有效性**

#### Prompt 變更
```
# 開頭新增
你是個量化交易員負責找出BTC 15分K線的最佳alpha。
你的目標是找到一個能在雙向市場（多頭和空頭）都獲利的策略，
而不是碰巧在牛市賺錢。
```

#### Prompt 新增要求
| 條款 | 內容 |
|------|------|
| 雙向獲利 | Long PF > 1.0 AND Short PF > 1.0（兩邊都要賺錢） |
| Exit 有效性 | TP + SL > 30%（不能只是 Timeout 等收盤） |
| max_holding_bars | 只是一個保險，不應是主要出场方式 |
| 雙向 PF 分析 | 每次測試後要分析 Long PF 和 Short PF |

#### 新增驗證步驟
- 步驟 5：驗證 Exit 條件（檢查 TP + SL 是否 > 30%）
- 步驟 6：驗證雙向獲利（檢查 Short PF 是否 > 1.0）

#### 原因
之前的策略 98.4% 靠 Timeout 出場，TP/SL 幾乎是裝飾品。Sharpe 66 但只是 BTC 牛市 beta，不是真正的 alpha。

---

### v1.1 — 2026-04-18 14:17
**強制每次測試後儲存**

#### Prompt 變更
```
# 新增嚴格規定
禁止跳過儲存！即使覺得結果不好也要儲存，方便未來檢視。

# 儲存格式改為
迭代_N_test_M.md
（區分：同一迭代的不同測試）
```

#### 原因
研究者有時不會儲存測試結果，導致歷史紀錄不完整。

---

### v1.0 — 2026-04-18 14:04
**初始版本**

#### 包含
- 研究者指南.md：完整流程、步驟、代碼模板
- backtest_core.py：回測引擎

#### 流程
1. 查看歷史（failures.md）
2. 設計進場邏輯
3. 實現 generate_signals()
4. 測試並分析結果
5. 儲存並記錄
6. 調整後繼續

#### 主要功能
- 研究者只需實現 generate_signals() 函數
- 回測引擎處理止損/止盈/成本
- 每次迭代後儲存策略代碼和描述
- 歷史追蹤（failures.md）
- 分析引導（讓 agent 自己判斷問題）

---

## 數據

- 檔案：`btcusdt_15m.parquet`
- 欄位：`open`, `high`, `low`, `close`, `volume`

## 回測引擎

- 位置：`backtest_core.py`
- 成本：0.03% 單邊（進場 + 出場 = 0.06%）
- 進場：信號在 bar t → `closes[t+1]` 進場
- 止損/止盈：相對於進場價計算

---

## 檔案結構

```
research_prompt/
├── README.md          # 本檔案（版本變更日誌）
├── 研究者指南.md      # 研究者 prompt
├── backtest_core.py  # 回測引擎
└── CHANGELOG.md      # 迭代詳細日誌（策略結果）
```

---

## 門檻（Current）

| 指標 | 門檻 |
|------|------|
| WR（勝率） | ≥ 60% |
| PF（盈虧比） | ≥ 1.8 |
| DD（最大回撤） | ≤ 25% |
| Sharpe | ≥ 1.5 |
| 月交易筆數 | ≥ 100 |
| **Exit 有效性** | TP + SL > 30% |
| **雙向獲利** | Long PF > 1.0 AND Short PF > 1.0 |

---

## 研究教訓 (Lessons Learned)

這些是從實際研究過程中得到的教訓，每次修改 Prompt 前都應該先看一遍。

### Lesson 1：Timeout 不是有效的 Exit
**發現時間**：v1.0 → v2.0

**問題**：策略靠 Timeout（98.4%）平倉，TP/SL 幾乎是裝飾品。

**教訓**：
- Timeout 只是「時間到就平倉」，不是真正的止盈止損
- 高 Timeout = 策略對市場沒有方向性判斷
- TP + SL 觸發率必須 > 30%，否則策略無效

**為何重要**：只靠 Timeout 的策略，在空頭市場會完全失效。

---

### Lesson 2：牛市 beta ≠ 策略 alpha
**發現時間**：v1.0 → v2.0

**問題**：Sharpe 高達 66（正常應在 0.5~3.0），36/36 配置全通過。

**教訓**：
- 如果任何參數組合都能賺錢，代表利潤來自市場本身（BTC 牛市），不是策略
- 必須要求「雙向獲利」— 空頭市場也要能賺錢
- 對比 Buy & Hold 才能知道真正的 alpha

**為何重要**：沒有真實 alpha 的策略，在熊市會爆虧。

---

### Lesson 3：每次測試都要儲存
**發現時間**：v1.0 → v1.1

**問題**：研究者有時跳過儲存，導致歷史紀錄不完整。

**教訓**：
- 歷史紀錄是未來決策的基礎
- 即使結果不好也要儲存（失敗也是數據）
- 用「迭代_N_test_M.md」區分同一迭代的不同測試

**為何重要**：沒有歷史 = 重複犯同樣的錯誤。

---

### Lesson 4：寬 SL 反而 WR 更高
**發現時間**：迭代 1

**問題**：SL=1.0% 時 WR=73.5%，SL=0.3% 時 WR=70.3%。

**原因**：BTC 15分K 市場噪聲大，窄止損被頻繁觃發。

**教訓**：
- 不能假設「窄止損 = 風險低」
- 要根據市場特性調整參數
- 先了解市場，再設計策略

---

### Lesson 5：進場邏輯比參數更重要
**發現時間**：迭代 1 → 迭代 2

**問題**：只靠調整 SL/TP 參數無法突破瓶頸。

**教訓**：
- 參數優化有上限
- 真正的突破來自更好的進場邏輯
- 1H Direction + 15m Momentum 比純 15m 指標更有效

---

### Lesson 6：Sharpe 66 的代價
**發現時間**：QA 驗證

**問題**：Sharpe 66 = 牛市特定環境的產物。

**代價**：
- 空頭市場 Sharpe 可能變負
- 連續亏损時 DD 會急劇擴大
- 高 Sharpe 不等於低風險

**教訓**：
- 不要只看 Sharpe，要看夏普的來源
- 要測試不同市場環境（多頭、空頭、橫盤）
- 最大回撤（DD）比 Sharpe 更重要
